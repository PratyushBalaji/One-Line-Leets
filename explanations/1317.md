...
As you can see, the list comprehension computes all the values, while the generator expression leaves them as it is. Only when you call `next()` on the generator does it evaluate what `2**0` means and returns the value `1`. This is the power of lazy evaluation, values are only computed when required, and left as is if not. While this computation is not too intensive, as more computation is involved, generator expressions become drastically faster.
